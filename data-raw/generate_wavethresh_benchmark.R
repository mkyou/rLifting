...
make_label = function(fam, fn)
  paste0(switch(fam,
    DaubExPhase = "db", DaubLeAsymm = "la", Coiflets = "co"), fn)
...
WAVELET_GRID$wavelet = mapply(make_label,
  WAVELET_GRID$family, WAVELET_GRID$fn)

POLICY_TYPE = rbind(
  expand.grid(policy = c("universal", "cv", "fdr"),
    type = c("soft", "hard"), stringsAsFactors = FALSE),
...
for (sig in DJ_SIGNALS) {
  for (ci in seq_len(nrow(CONFIGS))) {
    ...
    key = sprintf("%s__%s__%s__%s",
      sig, cfg$wavelet, cfg$policy, cfg$type)
...
run_job = function(job) {
  ...
  for (i in seq_len(N_SIM)) {
    ...
    res = tryCatch({
      mb = microbenchmark({
        ...
        th_obj = wavethresh::threshold(wd_obj, policy = cfg$policy,
          type = cfg$type)
  ...
  row = c(
    list(Signal = sig, Pkg = "wavethresh", Wavelet = cfg$wavelet,
         Boundary = paste0(cfg$policy, "_", cfg$type),
         N = sum(!is.na(mse_vec))),
...
message(sprintf("Final: %d rows x %d cols",
  nrow(benchmark_wavethresh), ncol(benchmark_wavethresh)))
save(benchmark_wavethresh, file = "data/benchmark_wavethresh.rda",
  compress = "xz")
message("Saved data/benchmark_wavethresh.rda")
