...
# Generates data/benchmark_nlt.rda
#
# Design: identical to adlift, replacing denoise() with denoiseperm().
#   The `per` (lifting permutation) is nlt's core stochastic mechanism —
#   kept at default (random per call). Each replica draws a fresh
#   permutation via the worker's L'Ecuyer-CMRG stream.
#
#   Configs    : 96  (pred x neigh x int x clo x rule = 4x3x2x2x2)
#   Reps       : 1000 per config x signal
#   Total rows : 96 x 4 = 384
#
# Checkpoint/resume: each job writes a 1-row .rds to data/tmp_nlt/.
# Runtime estimate: ~13 h on 11 cores (nlt ~1.3 s/call).
#
# source("data-raw/generate_nlt_benchmark.R")

suppressPackageStartupMessages({
  library(parallel)
  library(nlt)
  library(adlift)    # provides LinearPred, QuadPred, CubicPred, AdaptPred
...
run_job = function(job) {
  ...
  for (i in seq_len(N_SIM)) {
    ...
    res = tryCatch({
      mb = microbenchmark(
        out <- as.vector(nlt::denoiseperm(x_grid, noisy,
  ...
  row = c(
    list(Signal=sig, Pkg="nlt", Wavelet=cfg$pred_name,
...
benchmark_nlt = do.call(rbind, lapply(files, readRDS))
message(sprintf("Final: %d rows x %d cols", nrow(benchmark_nlt), ncol(benchmark_nlt)))
save(benchmark_nlt, file="data/benchmark_nlt.rda", compress="xz")
message("Saved data/benchmark_nlt.rda  — remove data/tmp_nlt/ when satisfied.")
