# inst/scripts/test_denoising_visual.R
...
run_visual_benchmark = function(signal_name, wavelet_name, ext = "symmetric") {
  ...
  # A. Offline
  out_off = denoise_signal_offline(noisy, sch, alpha, beta, "semisoft", ext)
  metrics_off = calc_metrics(clean, out_off)

  # B. Causal
  out_causal = denoise_signal_causal(
    noisy, sch, W, alpha, beta, "semisoft", ext
    )
  ...
  # Titulo com Metricas
  title_str = sprintf(
    "%s + %s (%s)\nInput: %.1fdB | Off: %.1fdB | Causal: %.1fdB",
    toupper(signal_name), toupper(wavelet_name), toupper(ext),
    metrics_in$snr, metrics_off$snr, metrics_causal$snr
  )

  # 4. Plot
  p = ggplot(df_long, aes(x = Index, y = Value, color = Type, alpha = Type)) +
    geom_line() +
...
# MATRIZ DE TESTES (Sinais x Ondaletas x Extensoes)
signals  = c("doppler", "heavisine", "bumps")
wavelets = c("haar", "db2", "cdf97")
# Testaremos 'symmetric' (Padrao) vs 'zero' (Ruim) para ver a diferenca
extensions = c("symmetric", "zero")

# Armazena plots para grid final
plot_list = list()
idx = 1

cat(sprintf("%-10s | %-8s | %-8s | %-9s | %-9s\n",
            "SINAL", "WAVELET", "EXT", "SNR(Off)", "SNR(Caus)", "RMSE(Caus)"))
...
for (sig in signals) {
  for (wav in wavelets) {
    for (ext in extensions) {
      ...
      cat(
        sprintf(
          "%-10s | %-8s | %-8s | %5.2f dB  | %5.2f dB  | %5.2f \n",
          sig, toupper(wav), ext,
          res$metrics$off$snr, res$metrics$causal$snr,
          res$metrics$causal$rmse
          )
        )

      # Guarda apenas o plot Symmetric para nao poluir visualmente
      if (ext == "symmetric") {
...
# Plota Grid 3x3 (Sinais nas linhas, Ondaletas nas colunas)
