...
public:
  ...
  double push_and_process(
    ...
    for(int j = levels - 1; j >= 0; j--) {
        ...
        if (step.type == "predict") {
          for(int i=0; i<n_odd; i++) {
            ...
            for(int m=0; m<k_filt; m++) sum += get_val(
              even, i + step.start_idx + m, n_even
            ) * step.coeffs[m];
            odd[i] += sum;
          }
        } else {
      ...
      for(int i=0; i<n_even; i++) if (2*i < target_size)
        work_approx[j][2*i] = even[i];
      for(int i=0; i<n_odd; i++) if (2*i+1 < target_size)
...
NumericVector run_causal_batch_cpp(
  ...
  for(int i=0; i<n; i++) output[i] = engine.push_and_process(
    signal[i], alpha, beta, method, i, i
  ); // Passamos 'i' como update_freq se quisermos bypass,
  // mas aqui mantivemos a assinatura
  return output;
}
