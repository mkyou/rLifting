...
Focaremos na aplicação prática de ondaletas para remoção de ruído (*denoising*), comparando a abordagem clássica (*offline*/não-causal/limiares globais) com a abordagem de tempo real (causal/limiares adaptativos).
...
Este método (do modo que é implementado no pacote `rLifting`) introduz dois fenômenos menos desejáveis:

-   Warm-up (Aquecimento): As primeiras amostras (tamanho da janela) não são filtradas (retorna o original), pois o algoritmo precisa acumular informação antes de começar a suavização.
...
# Nota: definimos levels=3 para igualar a profundidade do offline.
...
É fácil ver que ambos os métodos melhoram consideravelmente a relação entre o sinal e o ruído quando comparados ao sinal original.
O método não-causal, por poder "olhar para o futuro", é um pouco melhor neste processo.
Por outro lado, o método causal também é bastante eficiente em reduzir o ruído.
