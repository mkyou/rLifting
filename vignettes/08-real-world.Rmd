---
title: "08 — Real-world signal denoising: infant cardiac monitoring"
output:
  rmarkdown::html_vignette:
    toc: true
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{08 — Real-world signal denoising: infant cardiac monitoring}
...
> see `vignette("02-thresholding-and-tuning")` and `vignette("07-benchmarks")` §2–§3.
...
at production rates (see `vignette("07-benchmarks")` §6).
...
the hot path (`vignette("03-causal-stream")` §3).
...
- `vignette("03-causal-stream")` — warm-up dynamics, `update_freq` tuning, window-size selection.
- `vignette("05-irregular-grids")` — full irregular-grid API; per-wavelet benchmark table for seven irregular signals.
- `vignette("07-benchmarks")` — cross-package MSE and throughput comparison on the Donoho–Johnstone test signals.
