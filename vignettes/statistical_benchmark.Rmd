...
Este guia apresenta uma validação estatística estruturada das ondaletas implementadas no pacote `rLifting`.
...
1.  Utilizamos as funções `bumps` (transientes abruptos), `doppler` (frequência variável) e `HeaviSine` (saltos e ondas) para testar as ondaletas.
2.  Geramos $N = 1000$ curvas, com $T = 2048$ pontos discretos observados para cada uma das funções.
3.  Para cada uma das curvas, adicionamos ruídos aditivos, independentes, e gaussianos $(\sigma = 0.2)$.
4.  Para cada uma das funções, testamos todas as combinações de ondaletas, métodos de *thresholding*, modos de extensão e modos de operação nativos do pacote.

Avaliamos a eficácia das suavizações com base na qualidade do sinal recuperado (SNR — *Signal to Noise Ratio*) e na precisão global de reconstrução (RMSE — *Root Mean Squared Error*).
...
No modo não-causal, ondaletas suaves e longas (`CDF9/7` ou `DD4`) geralmente superam a ondaleta de Haar.
...
Vale dizer que, por conta do seu pequeno número de coeficientes, a ondaleta de Haar não sofre com este fenômeno como as demais.
...
O melhor método de *thresholding* dependerá da ondaleta utilizada e do sinal que está sendo suavizado.

## Conclusão

Com base neste guia, para um uso experimental/rápido/ingênuo, recomendamos que o leitor adote os seguintes padrões: 1.
Use suavização *offline*/não-causal, se puder.
Se o processamento em tempo real não for necessário, este modo oferece performance consideravelmente superior.
2.
Cuidado com a latência: em tempo real, ondaletas suaves `CDF97` ajudam a limpar o sinal, mas introduzem atraso.
Em nossos testes, a ondaleta de Haar performou melhor para todos os sinais considerados.
3.
Nossos testes consideraram funções específicas, e, sobretudo, ruídos específicos.
Se o desvio padrão dos ruídos fosse maior, por exemplo, é possível que teríamos uma maior discrepância entre as performances não-causais, causais e o *baseline*.
